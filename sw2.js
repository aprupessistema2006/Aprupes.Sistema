const CACHE_NAME = 'aprupes-sistema-v14'; // New cache to bypass old v13 SW lock
...
self.addEventListener('install', (event) => {
  console.log('[SW-v14] Installing new service worker (cache:', CACHE_NAME, ')');
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      console.log('[SW-v14] Precaching static assets');
...
self.addEventListener('activate', (event) => {
  console.log('[SW-v14] Activating new service worker');
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          ...
          .map((name) => {
            console.log('[SW-v14] Deleting old cache:', name);
            return caches.delete(name);
          })
      );
    }).then(() => self.clients.claim())
      .then(() => {
        console.log('[SW-v14] Activation complete, notifying clients to reload');
