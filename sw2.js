const CACHE_NAME = 'aprupes-sistema-v9'; // New cache to bypass old v8 SW lock
...
self.addEventListener('install', (event) => {
  console.log('[SW-v9] Installing new service worker (cache:', CACHE_NAME, ')');
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      console.log('[SW-v9] Precaching static assets');
...
self.addEventListener('activate', (event) => {
  console.log('[SW-v9] Activating new service worker');
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      ...
      .then(() => {
        console.log('[SW-v9] Activation complete, notifying clients to reload');
