...
class UpdateNotifier {
  constructor() {
    ...
    this.versionParam = '?v=20260925-1247';
  ...
  async checkVersionOnPageLoad() {
    try {
      const response = await fetch('version.json?v=20260925-1247');
      ...
      if (!response.ok) {
        ...
        if (!storedVersion || storedVersion !== '20260925-1247') {
          this.showUpdateBanner();
        }
        return;
      }
  ...
  async applyUpdate() {
    ...
    window.location = window.location.href.split('?')[0] + '?v=20260925-1247';
  }
